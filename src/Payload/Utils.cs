...
namespace Payload
{
    public class Utils
    {
        ...
        static public IDictionary<string, object> JSONFlatten(dynamic val)
        {
            return JSONFlatten(new Dictionary<string, object>(), null, val);
        }
        ...
        static public IDictionary<string, object> JSONFlatten(IDictionary<string, object> obj, string parent_key, dynamic val)
        {
            ...
            {
                foreach (var item in (IDictionary<String, Object>)val)
                {
                    var child_key = parent_key == null ? item.Key : parent_key + "[" + item.Key + "]";
                    JSONFlatten(obj, child_key, item.Value);
                }
            }
            else if (val is IList<dynamic>)
            {
                for (int i = 0; i < ((IList<dynamic>)val).Count; i++)
                {
                    var child_key = "[" + i + "]";
